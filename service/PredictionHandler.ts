...
export const handleNewMeasurementHook = async (hourId: string) => {
  ...
  getCalculatedPrediction(hour).catch(console.log);
};
const getCalculatedPrediction = async (hour: IHour) => {
  const today = new Date();
  const tempSensors = hour.sensors.filter(
  ...
  if (tempSensors && tempSensors[0]) {
    ...
    persistPredictionByHourID(hour.id, calcPrediction).catch(console.log);
  }
};

export const getCalculationPure = async (
  month: number,
  day: number,
  hour: number,
  temperature: number,
  ...
  console.log(requestURL);
  try {
    return await rp(requestURL);
  } catch (e) {
    console.log('Error requesting pure calculation');
    return Error('Ops cannot use my brain');
  }
};
